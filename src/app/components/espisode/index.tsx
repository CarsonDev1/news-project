								...
								{dataEpisodes?.server_data?.map((ep: any, index: number) => (
									<Link
										href={{
											...
											query: { episode: ep.name, link: ep.link_embed },
										}}
										key={index}
									>
										<button className='w-full aspect-[3/1] flex items-center justify-center rounded bg-gradient-to-b from-[#1A1F2E] to-[#1A1F2E] text-gray-300 hover:text-white border border-[#2A3041] transition-colors duration-200 text-sm'>
